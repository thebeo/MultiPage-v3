...
\NewDocumentCommand\ARDraw{mmmm}{
    ...
    {% path
        ...
        \IfStrEq{#4}{} % if beat
        {% No beat
            \path[draw=#3,line width=\LW,->] (S) to[bend right] (E);
        }
        {% beat
            \path[draw=#3,line width=\LW,->] (S) to[bend right] node[fill=white,inner sep=0pt,sloped,midway]{#4} (E);
        } 
    }
}
...
\NewDocumentCommand\ALDraw{mmmm}{
    ...
    }
    {
        ...
        \IfStrEq{#4}{} % if beat
        {% No beat
            \path[draw=#3,line width=\LW,->] (S) to[bend left] (E);
        }
        {% beat
            \path[draw=#3,line width=\LW,->] (S) to[bend left] node[fill=white,inner sep=0pt,sloped,midway]{#4} (E);
        } 
